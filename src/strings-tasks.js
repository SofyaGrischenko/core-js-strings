...
function isPalindrome(str) {
  const string = str.replace(/[^\w]/gi, '');
  const formedStr = string.toLowerCase();

  let end = formedStr.length - 1;
  for (let i = 0; i < formedStr.length / 2; i += 1) {
    if (formedStr[i] !== formedStr[end]) return false;
    end -= 1;
  }
  return true;
}
...
function findLongestWord(sentence) {
  ...
  return sentence.slice(index - max, index);
}
